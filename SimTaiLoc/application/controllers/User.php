...
class User extends CI_Controller{
	...
	public function index(){
		$this->load->view('user_/login.php');
	...
	public function login(){
		...
		if($this->form_validation->run() == TRUE){
			...
			$a_UserChecking = $this->authentication_model->a_fCheckUsers( $a_UserInfo );
			if($a_UserChecking){
				$this->session->set_userdata('user', $a_UserChecking);			 
				redirect(base_url('index.php/khachhang_showdata')); 
			}else{
				$this->b_Check = false;
			}
		}
		$a_Data['b_Check']= $this->b_Check;
		$this->load->view('user_/login.php', $a_Data);
	...
	public function register(){
		...
		// $this->form_validation->set_rules('sodt', 'lang:sodt', 'required|min_length[10]|max_length[10]');
		$this->form_validation->set_rules('sodt', 'lang:sodt', 'required|min_length[10]|max_length[10]');
		$this->form_validation->set_rules('dieukhoan', 'lang:dieukhoan', 'required');
		...
		if($this->form_validation->run() == TRUE){
			...
			$this->addData_model->insert_user($username,$password,$email,$sodt);
		...
		$this->load->view('user_/register.php', $data);
